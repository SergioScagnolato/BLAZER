...
const int PORTA1_FG         = D3;
const int PORTA2_FG         = D4; 
const int PORTA2_DSP        = D1;  //sda
const int PORTA1_DSP        = D2;
const int SINAL_RPM         = D8;
const int SINAL_REMOTO      = D0;
const int RELE_TRAVA_PORTA  = D7;  
...
void loop() {
 ...
 if(remoto==1 or acessoweb==1) { 
  ...
  if (acessoweb==1) lcd.clear(); 

   status_stdy=0;
   lcd.backlight();
   digitalWrite(LED_STDBY,LOW);

   if(leitorok!=1 and acessoweb==0 and MODOFUNCIONAMENTO==0) {
    lcd.setCursor(0,0);
    lcd.print("Leitor desconec");
    lcd.setCursor(3,1);
    lcd.print("Use WiFi");
   }
   ...
   if (acesso==1) {    // cadastro reconhecido
    mostra_hora();
    if(estadoIgnicao==0){
