...
// formata a string com zeros a esquerda
 char* formatar(char* txt,char *result) { 
  ...
  while(indice< tamasec) {
   if(txt[indice++] =='1') *retorno++='1'; //*retorno++=(byte)3;
   else *retorno++='0';   //*retorno++=(byte)2;
  }
  *retorno++='\0';
  return retorno;
 }
...
display.flipScreenVertically();
 ...
 display.setColor(WHITE);
  display.drawString(59, 10, "BLAZER CCY 5030");
  display.setFont(ArialMT_Plain_10);
  display.drawString(64, 28, "Sergio");
  display.drawString(61, 42, "Scagnolato");
  display.display();
  delay(4000);
...
// verifica se o leitor bio esta conectado
 if (finger.verifyPassword()) {
  ...
  display.drawString(24, linha_init, "** Leitor nao encontrado **");
 }
 ...
 display.drawString(21, linha_init, String(cadastros));
 display.drawString(57, linha_init, "Cadastros");
 display.display();
 linha_init+=ESPACO_LINHAS;
...
//  }
 ...
 if(leitorok==1 or MODOFUNCIONAMENTO==1) { 
  if(acesso == 0 and acessoweb==0) {
   ...
   display.setFont(ArialMT_Plain_24);
   display.clear();
   display.drawString(64, 3, "Aguardando");
...
//   wifion=0;
   ...
   display.clear();
   display.setFont(ArialMT_Plain_24);
   display.drawString(57, 20, "Stand By");
   display.display();
   status_stdy=1;
   display.setFont(ArialMT_Plain_10);
...
void mostra_hora() {
  ...
  display.setFont(ArialMT_Plain_24);
  display.drawString(65, 10, horario);
  } else {
   ...
   display.setFont(ArialMT_Plain_16);
   display.drawString(65,4,horafinal);
   display.drawString(65,21,minutofinal);
  }
}
...
void mostra_temperatura() {
  float temperatura = le_temperatura();
  char temp[33];
  sprintf(temp,"Core : %3.1f C",temperatura);
  display.setFont(ArialMT_Plain_10);
  display.drawString(39, 52, temp);
  display.display();
}
